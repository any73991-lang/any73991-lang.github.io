...
d('c2c_my_ads',   {'zh-CN':'我的广告','zh-TW':'我的廣告',en:'My Ads',ja:'私の広告',ko:'내 광고',es:'Mis Anuncios',ru:'Мои объявления',fr:'Mes Annonces',de:'Meine Anzeigen',pt:'Meus Anúncios',vi:'Quảng cáo của tôi',th:'โฆษณาของฉัน'});
...
d('c2c_min_placeholder',{'zh-CN':'最小金额','zh-TW':'最小金額',en:'Min Amount',ja:'最小額',ko:'최소 금액',es:'Monto mín.',ru:'Мин. сумма',fr:'Montant min.',de:'Mindestbetrag',pt:'Valor mín.',vi:'Số tiền tối thiểu',th:'จำนวนขั้นต่ำ'});
...
// ====== 法币名称 ======
d('fiat_cny', {'zh-CN':'人民币','zh-TW':'人民幣',en:'Chinese Yuan',ja:'人民元',ko:'위안',es:'Yuan Chino',ru:'Китайский юань',fr:'Yuan Chinois',de:'Chinesischer Yuan',pt:'Yuan Chinês',vi:'Nhân Dân Tệ',th:'หยวนจีน'});
d('fiat_hkd', {'zh-CN':'港币','zh-TW':'港幣',en:'Hong Kong Dollar',ja:'香港ドル',ko:'홍콩 달러',es:'Dólar HK',ru:'Гонконгский доллар',fr:'Dollar HK',de:'HK-Dollar',pt:'Dólar HK',vi:'Đô la Hồng Kông',th:'ดอลลาร์ฮ่องกง'});
d('fiat_twd', {'zh-CN':'新台币','zh-TW':'新台幣',en:'New Taiwan Dollar',ja:'台湾ドル',ko:'대만 달러',es:'Dólar Taiwanés',ru:'Тайваньский доллар',fr:'Dollar Taïwanais',de:'Taiwan-Dollar',pt:'Dólar Taiwanês',vi:'Đô la Đài Loan',th:'ดอลลาร์ไต้หวัน'});
d('fiat_usd', {'zh-CN':'美元','zh-TW':'美元',en:'US Dollar',ja:'米ドル',ko:'미국 달러',es:'Dólar EE.UU.',ru:'Доллар США',fr:'Dollar US',de:'US-Dollar',pt:'Dólar EUA',vi:'Đô la Mỹ',th:'ดอลลาร์สหรัฐ'});
d('fiat_jpy', {'zh-CN':'日元','zh-TW':'日圓',en:'Japanese Yen',ja:'日本円',ko:'일본 엔',es:'Yen Japonés',ru:'Японская иена',fr:'Yen Japonais',de:'Japanischer Yen',pt:'Iene Japonês',vi:'Yên Nhật',th:'เยนญี่ปุ่น'});
...
const API_BASE = location.hostname === 'any73991-lang.github.io'
  ? 'https://c4aae4890ed645b08c01f520e60ad318.codebuddy.cloudstudio.run'
  : '';
...
// ========== C2C 交易 ==========
const FIAT_SYMBOLS = {
  CNY: '¥', HKD: 'HK$', TWD: 'NT$', USD: '$',
  JPY: '¥', KRW: '₩', RUB: '₽', VND: '₫'
};
function getFiatSymbol() { return FIAT_SYMBOLS[C2C.fiatCurrency] || '¥'; }
function getFiatLabel() { return getFiatSymbol() + ' ' + C2C.fiatCurrency; }
...
async function loadC2CAds() {
  ...
  try {
    ...
    if (C2C.fiatCurrency) params += '&fiat=' + C2C.fiatCurrency;
    ...
    body.innerHTML = ads.map(function(a) {
      ...
      return '<div class="c2c-ad-row" data-ad-id="' + a.id + '" data-user-id="' + a.user_id + '">' +
        ...
        '<div class="c2c-ad-action">' +
          (mine ? '<button class="btn btn-xs btn-outline c2c-cancel-ad-btn" data-id="' + a.id + '">' + t('c2c_cancel_order') + '</button>' :
...
function openCreateAd() {
  ...
  $('c2c-ad-coin').value = C2C.filterCoin || 'USDT';
  $('c2c-ad-price').value = '';
  $('c2c-ad-min').value = '';
  $('c2c-ad-max').value = '';
  $('c2c-ad-remark').value = '';
...
async function submitAd() {
  ...
  try {
    await api.post('/api/c2c/ads', {
      ...
      fiat: C2C.fiatCurrency,
      network: C2C.filterNetwork !== 'ALL' ? C2C.filterNetwork : ''
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 法币标签点击
  $$('.c2c-token-tab').forEach(function(b) {
    b.addEventListener('click', function() {
      $$('.c2c-token-tab').forEach(function(x) { x.classList.remove('active'); });
      this.classList.add('active');
      C2C.fiatCurrency = this.dataset.fiat;
      loadC2CAds();
    });
  });

  // 币种/网络标签点击
  $$('.c2c-coin-tab').forEach(function(b) {
    b.addEventListener('click', function() {
      $$('.c2c-coin-tab').forEach(function(x) { x.classList.remove('active'); });
      this.classList.add('active');
      C2C.filterCoin = this.dataset.coin;
      C2C.filterNetwork = this.dataset.network || 'ALL';
      loadC2CAds();
    });
  });
